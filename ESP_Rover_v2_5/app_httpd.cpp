...
extern void leftMotor();
extern void rightMotor();
...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  while (true) {
    ...
    frame_time /= 1000;
    if (frame_time > 0) {
      currentFPS = (int)(1000.0f / frame_time + 0.5f);
...
static esp_err_t status_handler(httpd_req_t *req) {
  ...
  *p++ = '{';
  ...
  p += snprintf(
    ...
    ",\"fps\":%d,\"rssi\":%d,\"uptime\":\"%02u:%02u:%02u\"",
    currentFPS,
    currentRSSI,
    hrs,
    mins,
...
<body>
    ...
    <div class="header">
        KartX FPV Rover <span style=" float:center; font-size:14px; color:#00ff66;">● ONLINE </span>
    </div>
    <div class="status">
    ...
    <div class="flashPanel">
        ...
        <div class="sliderBox">
            ...
            <input id="brightness" type="range" min="0" max="255" value="180">
    ...
    <script>
        ...
        slider.oninput = function () {
            fetch("/brightness?val=" + this.value);
        };
        ...
        async function updateStats() {
            try {
                ...
                document.getElementById("wifi").innerHTML =
                    wifiIcon + " " + wifiText + " (" + data.rssi + " dBm)";
...
static esp_err_t flash_handler(httpd_req_t *req) {
  ...
  ledcWrite(FLASH_LED_PIN, flashState ? led_duty : 0);
...
static esp_err_t brightness_handler(httpd_req_t *req) {
  ...
  if (httpd_query_key_value(buf, "val", value, sizeof(value)) == ESP_OK) {
    ...
    if (flashState)
      ledcWrite(FLASH_LED_PIN, led_duty);
  }
